...
(function () {
  ...
  if (form) {
    var rules = {
      ...
      email: function (v) {
        if (!v.trim()) return "We need an email to send the itinerary.";
        if (!/^[^\s@]+@[^\s@]+\.[^\s@]{2,}$/.test(v.trim())) return "That email doesn't look right.";
        return "";
      },
      phone: function (v) {
        var digits = v.replace(/\D/g, "");
        if (!digits) return "A phone number helps us reach you faster.";
        if (digits.length < 8 || digits.length > 15) return "Please enter a valid phone number.";
        return "";
      },
      destination: function (v) {
        if (!v) return "Pick a destination — or 'not sure yet'.";
        return "";
      }
    };
    ...
    function setError(field, message) {
      ...
      var slot = $('[data-error-for="' + field.id + '"]', wrapper);
    ...
    form.addEventListener("submit", function (e) {
      ...
      // No backend here — wire this up to your form handler / CRM.
